...
```{r, warning = F, message = F}
demo_full <- rio::import("Data/Prepped Data/Demographics/demo_cleaned.csv") %>%
  dplyr::mutate(
    fam_os = factor(
      fam_os,
      levels = c(
    ...
    use_os = factor(use_os, levels = c("Never", "Rarely", "Sometimes", "Often")),
    focus_group = factor(focus_group, levels = c("R1", "Non-R1"))
  )
...
## Supplemental Figure 1 - Research Area Plot
...
ggsave("Figures/Figure S1_Research Areas.png", plot = research_plot)

```


## Figure 1
### Familiarity of Open Science
...
fam_plot_altTextCount <- demo_full |>
  dplyr::select(id, focus_group, fam_os) |>
  dplyr::distinct() |>
  dplyr::group_by(focus_group, fam_os) |>
  dplyr::summarize(n = n())

use_plot_altTextCount <- demo_full |>
  dplyr::select(id, focus_group, use_os) |>
  dplyr::distinct() |>
  dplyr::group_by(focus_group, use_os, .drop = F) |>
  dplyr::summarize(n = n(), .groups = "drop")

ggsave("Figures/Figure 1_familiarity and use of OS.png", plot = ratings_plot, width = 8, height = 5, units = "in", bg = "white")
rio::export(fam_plot_altTextCount, "Figures/Figure 1.1_Alt text counts for Familiarity of Open Science Practices.csv")
rio::export(use_plot_altTextCount, "Figures/Figure 1.2_Alt text counts for Use of Open Science Practices.csv")
```
