...
```{r}
transcript_R1_1 <- base::readLines("Data/Raw Data/Transcripts/MASTER_R1_Transcript_2023_05_15.txt") %>%
  as.data.frame() %>%
  ...
  dplyr::mutate(Transcript = "R1_1",
                Participant = sub("\\:.*", "", Comment)) %>%
  dplyr::relocate(Comment, .after = Participant)

transcript_R1_2 <- base::readLines("Data/Raw Data/Transcripts/MASTER_R1_transcript_2023_05_22.txt") %>%
  as.data.frame() %>%
  dplyr::rename(Comment = 1) %>%
  dplyr::filter(Comment != "") %>%
  dplyr::filter(grepl(pattern = ":",
...
transcript_nonR1_1 <- base::readLines("Data/Raw Data/Transcripts/MASTER CONSENSUS_NonR1_Transcript_2023_07_20.txt") %>%
  as.data.frame() %>%
  dplyr::rename(Comment = 1) %>%
  dplyr::filter(Comment != "") %>%
  dplyr::filter(between(row_number(),3,171)) %>%
